define redis_source(
    ...
    exec { "make ${version}":
         command => "/bin/sh -c 'cd ${path}/redis_${version} && make && mv redis-server ${bin}/ && mv redis-cli ${bin}/ && mv redis-benchmark ${bin}/ && mv redis-check-dump ${bin}/'",
    ...
    file { "/etc/init/redis-server.conf":
         content => template("redis/templates/redis-server.erb"),
         owner => root,
         group => root,
         mode => 744,
    }
    file { "/etc/redis.conf":
        ...
        content => template("redis/templates/redis.conf.erb"),
        replace => false;
    }
}
